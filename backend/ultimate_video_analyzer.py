...
class UltimateVideoAnalyzer:
    ...
    def _analyze_text_for_violations(self, text: str) -> Dict:
        """Analyze text for policy violations using keyword matching"""
        if not text:
            return {}
...
async def main(analyzer, video_path, args):
    ...
    if audio.get("policy_flags"):
        for polcat, polinfo in audio["policy_flags"].items():
            # Use RAG to get policy explanation for audio
    ...
    # Print detailed violation explanations with RAG policy details
    if violation_explanations:
        safe_print("\n🔎 DETAILED POLICY VIOLATION EXPLANATIONS (RAG):")
        for v in violation_explanations:
            if v["type"] == "visual":
                safe_print(f"- Frame {v['frame']} (t={v['timestamp']:.1f}s): {v['category']} detected by model (confidence {v['confidence']:.2f})")
                if v['blip']:
                    safe_print(f"    BLIP: {v['blip']}")
                if v['ocr']:
                    safe_print(f"    OCR: {v['ocr']}")
                if v['policy']:
    ...
    # Save report with structured RAG explanations
